<template>
  <div>
    <el-form
      :model="editForm"
      ref="formRef"
      :rules="rules"
      label-position="left"
      label-width="80px"
      style="width: 500px; margin-left: 50px"
    >
      <el-form-item required label="商品类型" prop="incomePayType">
        <el-select
          v-model="editForm.incomePayType"
          class="filter-item"
          placeholder="请选择商品类型"
        >
          <el-option label="鲜花" value="1" />
          <el-option label="花束" value="2" />
          <el-option label="礼盒" value="3" />
          <el-option label="花篮" value="4" />
          <el-option label="绿植" value="5" />
          <el-option label="周花" value="6" />
        </el-select>
      </el-form-item>
      <el-form-item required label="商品名称" prop="name">
        <el-input
          v-model="editForm.name"
          type="datetime"
          placeholder="请输入商品名称"
        />
      </el-form-item>
      <el-form-item required label="店铺价" prop="price">
        <el-input v-model="editForm.price" placeholder="请输入商品价格" />
      </el-form-item>
      <el-form-item required label="市场价" prop="current_price">
        <el-input
          v-model="editForm.current_price"
          placeholder="请输入商品数量"
        />
      </el-form-item>
      <el-form-item label="备注" prop="attach">
        <el-input
          v-model="editForm.attach"
          :autosize="{ minRows: 2, maxRows: 4 }"
          type="textarea"
          placeholder="请输入备注"
        />
      </el-form-item>
      <el-form-item>
        <el-button @click="closeTag()">取消</el-button>
        <el-button type="primary" @click="updateForm()">确认</el-button>
      </el-form-item>
    </el-form>
  </div>
</template>

<script>
import { fetchListById, updateCash } from '@/api/goods'
import { defineComponent, reactive, toRefs, ref } from 'vue'
import useCloseTag from '@/hooks/useCloseTag'
import { ElMessage } from 'element-plus'

export default {
  props: ['id'],
  setup(props) {
    const { closeTag } = useCloseTag()
    const state = reactive({
      rules: {
        incomePayType: [
          { required: true, message: '商品类型必填', trigger: 'change' },
        ],
        name: [
          {
            required: true,
            message: '商品名称必填',
            trigger: 'blur',
          },
        ],
        price: [{ required: true, message: '商品价格必填', trigger: 'blur' }],
        current_price: [
          { required: true, message: '商品市场价必填', trigger: 'blur' },
        ],

        pay: [{ required: true, message: '商品数量必填', trigger: 'blur' }],
        remarks: [],
        attach: [],
      },
      editForm: {
        incomePayType: '',
        name: '',
        price: '',
        current_price: '',
        pay:'',
        attach: '',
        // accoutCash: '',
        // remarks: '',
      },
    })

    //页面ref的引用
    const formRef = ref(null)
    const getCashData = async () => {
      let result = await fetchListById(props.id)
      state.editForm = result.data
    }
    getCashData()

    //更新数据
    const updateForm = () => {
      formRef.value.validate(valid => {
        if (valid) {
          console.log(state.editForm, 'sssssssssssssssss')
          updateCash(state.editForm).then(response => {
            //返回上个页面
            ElMessage({
              message: '更新成功',
              type: 'success',
            })
            //1秒后返回上个页面
            setTimeout(() => {
              closeTag.value({ reload: true })
            }, 1000)
          })
        }
      })
    }

    return { ...toRefs(state), closeTag, formRef, updateForm }
  },
}
</script>